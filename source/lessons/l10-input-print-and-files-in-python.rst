...
تابع ``()input`` [`اسناد پایتون <http://docs.python.org/3/library/functions.html#input>`__] یک سطر از کاربر را می‌خواند و در قالب یک شی از نوع ``str`` برمی‌گرداند::
...
تابع ``()__import__`` [`اسناد پایتون <http://docs.python.org/3/library/functions.html#__import__>`__] عملکردی برابر با دستور ``import`` دارد ولی با این تفاوت که می‌توان از آن به شکلی پویا در طی اجرای برنامه برای وارد کردن ماژول‌های گوناگون استفاده نمود؛ در این حالت نام یک ماژول حتی می‌تواند به شکل یک متغیر در آرگومان تابع قرار بگیرد.
...
پیش‌تر با تابع ``()print`` پایتون آشنا هستیم ولی در اینجا می‌خواهیم کمی بیشتر آن را بررسی نماییم. این تابع [`اسناد پایتون <http://docs.python.org/3/library/functions.html#print>`__]؛ الگویی به شکل پایین دارد::
